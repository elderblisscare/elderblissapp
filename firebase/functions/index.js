...
exports.sendEmergencySmsViaProvider = functions.https.onCall(async (data) => {
  ...
  if (!accountSid || !authToken || !fromNumber) {
    return {
      success: false,
      message:
        "Backend SMS provider is not configured. Set TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, TWILIO_FROM_NUMBER.",
      sentContacts: [],
  ...
  return {
    ...
    message:
      failedContacts.length === 0
        ...
        : `Backend SMS sent to ${sentContacts.length} contact(s), failed for ${failedContacts.length}.`,
    sentContacts,
    failedContacts,
  };
});
